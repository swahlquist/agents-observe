...
<task type="auto" tdd="false">
  ...
  <files>
    ...
    - app/client/src/types/index.ts (extend RecentSession: add five new derived fields only; lastActivity stays `number` because Plan 01 coerces the wire value)
  </files>
  <read_first>
  ...
  <behavior>
    ...
    - Status badge palette: WORKING green, WAITING_FOR_INPUT amber, WAITING_ON_PERMISSION red, IDLE muted gray, FINISHED blue, ABANDONED dim gray with slight transparency. Per CONTEXT.md § "Card visuals". For WAITING_ON_PERMISSION, append the tool name from `statusDetail` to the badge label (e.g. "Waiting on Bash"). For IDLE, append elapsed time (e.g. "Idle 15m") per HOME-13 computed as `Date.now() - session.lastActivity`. No client-side null guard needed: Plan 01 specifies that the server coerces the wire field via `(row.last_activity ?? row.started_at)` before serializing, so `session.lastActivity` is always a non-null number on the wire. This is the Round 3 New-H mitigation: the previous patch revision considered widening the client type to `number | null` and adding a client-side fallback chain, but that approach would have required null guards at three additional in-repo client call sites (`labels-modal.tsx:346` sort, `labels-modal.tsx:626` and `session-modal.tsx:347` `formatRelativeTime` calls). Server coercion is the cleaner fix.
    ...
    - Extend `RecentSession` in `app/client/src/types/index.ts` with five new required fields: `derivedStatus: 'WORKING' | 'WAITING_FOR_INPUT' | 'WAITING_ON_PERMISSION' | 'IDLE' | 'FINISHED' | 'ABANDONED'`, `statusDetail: string | null`, `needsYou: boolean`, `lastActionLabel: string | null`, `lastActionAt: number | null`. KEEP the existing `status: string` (legacy 2-state, `'active' | 'ended'`) field unchanged on the type; Plan 01 leaves it on the wire because 7+ in-repo consumers (sidebar Unassigned bucket, settings tabs, modals, labels, session-list.tsx kept alive per CONTEXT.md § "Things to leave alone") still compare `session.status === 'active'` and would break under a narrowing. Phase 1b migrates them and drops the field.
  </behavior>
  <action>
    Create `session-card.tsx` first. Define a small `categoryIcon(intent)` helper inside the file (or in a sibling `session-card-helpers.ts` if it grows past 40 lines) that maps intent keywords to lucide-react icon components per CONTEXT.md § "Card visuals". Define a `colorStripeIndex(sessionId)` helper that returns 0..7 from a stable hash of the session id. Define a `STATUS_BADGE` map keyed by the six SessionStatus values (this maps from `session.derivedStatus`, NOT the legacy 2-state `session.status`), with `label` and `className` (Tailwind class for color). For WAITING_ON_PERMISSION, the label concatenates "Waiting on " + statusDetail when present. For IDLE, append elapsed time computed from `lastActivity`. Reuse the existing `formatRelativeTime` helper from `session-list.tsx` (copy or extract to a shared module; extract if both `session-list` and `session-card` need it; otherwise inline a fresh copy here, since CONTEXT.md § "Things to leave alone" calls out that `session-list.tsx` still backs ProjectView until Phase 1b and should not be deleted).
  ...
  <acceptance_criteria>
    ...
    - `RecentSession` in `app/client/src/types/index.ts` carries the five new fields (`derivedStatus` six-state union, `statusDetail`, `needsYou`, `lastActionLabel`, `lastActionAt`) as required (non-optional), AND the legacy `status: string` field remains present and unchanged, AND the `lastActivity: number` field is unchanged (NOT widened to `number | null`; Plan 01's wire-level coercion handles the underlying NULL case). Confirm via `grep -nE "^\s*(status|derivedStatus|lastActivity):" app/client/src/types/index.ts`. All three lines must appear; the `lastActivity` line must remain `number` (no `| null`).
    - SessionCard renders the IDLE badge with non-NaN elapsed time. Add a Vitest unit test in a new `app/client/src/components/main-panel/session-card.test.tsx`: given a session fixture with `derivedStatus: 'IDLE'` and `lastActivity: Date.now() - 15 * 60 * 1000` (the post-coercion wire shape; server already substituted `started_at` if the underlying column was NULL), the badge text contains "15m" (or rounded equivalent), NOT "NaN". The server-side coercion regression (input row with `last_activity = null` produces wire `lastActivity = started_at`) is covered in Plan 01 acceptance criteria, not here.
...
<task type="auto" tdd="false">
  ...
  <behavior>
    ...
    - Group sessions by `projectId`. Sessions with `projectId === null` are placed under a synthetic "Unassigned" group with `projectName: "Unassigned"`. Inside a group, separate active from finished-today by derivedStatus: `derivedStatus === 'FINISHED'` with `stoppedAt` after local midnight goes to that group's `finishedTodaySessions`; everything else (`derivedStatus !== 'FINISHED'`) goes to `activeSessions`. (Finished sessions older than local midnight go to a global "older" bucket that is dropped from the home view; the 30-session window already trims them.) Note: this uses the new `derivedStatus` field, NOT the legacy `status` field (which is still `'active' | 'ended'`).
  ...
  <action>
    ...
    The Overview tab placeholder card is a small `<div>` containing: intent (or "(no intent set)" if null), `lastActionLabel` (or "no recent action"), the status badge (reuse the `STATUS_BADGE` map from `session-card.tsx`; extract to a shared helper file `session-status-badge.tsx` if both files now need it), and the literal string "Tasks: arriving in Phase 1b". The card fetches the session via `useRecentSessions()` (the same hook the home page uses; Plan 01 makes it carry the new derived fields `derivedStatus`, `statusDetail`, `needsYou`, `lastActionLabel`, `lastActionAt`) and reads the matching row by `session.id`. Do NOT use `useSessions(projectId)` for the Overview card: that hook calls `/api/projects/:id/sessions`, which Phase 1a does NOT rewire to emit the new fields (H2 mitigation per adversary review; rewiring those routes is deferred to Phase 1b). For sessions outside the home view's 30-session window (i.e. not present in `useRecentSessions` data), the Overview placeholder renders the literal string `"No recent activity. Open the Activity tab for full timeline."` in place of the derived-field card; the Activity tab continues to work via the existing `useEvents` hook regardless.
  ...
  <acceptance_criteria>
    ...
    - The Overview tab placeholder card in `main-panel.tsx` reads from `useRecentSessions` (NOT `useSessions(projectId)`). Confirm via `grep -n "useRecentSessions\|useSessions" app/client/src/components/main-panel/main-panel.tsx`: the Overview placeholder code path must reference `useRecentSessions`. This is the H2 mitigation: `/api/projects/:id/sessions` is not rewired in Phase 1a to carry the new derived fields.
