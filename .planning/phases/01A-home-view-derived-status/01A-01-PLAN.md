...
must_haves:
  truths:
    ...
    - "Derivation cost is structurally bounded to `1 + N` queries per `/sessions/recent` request, with `N <= 30` (the SELECT in `getRecentSessions`) and each per-row event lookup capped at `LIMIT 50` (the new `getRecentEventsForSession` helper). No unbounded scan and no JOIN fan-out. (Specific wall-clock latency is intentionally not promised as a contract; if a hard SLA is needed later, add a microbenchmark task; H4 adversary disposition.)"
